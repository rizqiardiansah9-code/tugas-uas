    ...
    <script>
        function tradeApp() {
            return {
                ...
                get myTotal() {
                    return this.myOffer.reduce((acc, i) => acc + parseFloat(i.price), 0);
                },
                get theirTotal() {
                    return this.theirOffer.reduce((acc, i) => acc + parseFloat(i.price), 0);
                },
                ...
                getRarityColor(item) {
                    const rKey = item.metadata?.rarity || 'common';
                    const colors = {
                        'uncommon': '#16a34a',
                        'rare': '#3b82f6',
                        'mythical': '#7c3aed',
                    ...
                    return colors[rKey] || colors.common;
                },

                getRarityText(item) {
                    const rKey = item.metadata?.rarity || 'common';
                    return rKey.charAt(0).toUpperCase() + rKey.slice(1).replace('_', ' ');
                },

                getRarityTextColor(item) {
                    const rKey = item.metadata?.rarity || 'common';
                    return rKey === 'legendary' ? '#000' : '#fff';
                },
    ...
    <div class="relative overflow-hidden -mx-6"
        ...
        <div class="grid grid-cols-[1fr_200px_1fr] gap-4 px-6 py-6 items-start">
            ...
            <div class="bg-[#12141c]  overflow-hidden flex flex-col">
                <!-- Header -->
                <div class="flex items-center justify-between p-4 bg-[#151823] border-b border-white/5 cursor-pointer select-none"
                    @click="offerOpen = !offerOpen">
                    <div class="flex items-center gap-3">
                        <i class="fas fa-shopping-cart text-white"></i>
                        <span class="text-white font-bold text-lg" x-text="formatPrice(myTotal)"></span>
                        <span class="bg-[#4c6fff] text-white text-xs font-bold px-2 py-0.5 rounded"
                            x-text="myOffer.length" x-show="myOffer.length > 0"></span>
                    </div>
                    <div class="flex items-center gap-4">
                        <button
                            class="text-xs text-[#aab0d5] hover:text-white px-2 py-1 rounded bg-[#1c202e] hover:bg-[#252a3d] transition-colors"
                            @click.stop="clearOffer()" x-show="myOffer.length > 0">
                            Clear
                        </button>
                        <button
                            class="w-6 h-6 rounded bg-[#1c202e] hover:bg-[#252a3d] flex items-center justify-center text-gray-400 hover:text-white transition-colors"
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <i class="fas fa-chevron-down text-gray-400 text-xs transition-transform duration-200"
                                :class="{ 'rotate-180': !offerOpen }"></i>
                        </div>
                    </div>
                </div>

                <!-- Body -->
                <div x-show="offerOpen" x-transition
                    class="p-4 bg-[#12141c] overflow-y-auto no-scrollbar transition-all duration-300"
                    :class="offerExpanded ? 'min-h-[300px] max-h-[500px]' : 'min-h-[150px] max-h-[200px]'">
                    <p class="text-gray-400 text-sm text-center py-8" x-show="myOffer.length === 0">What items do you
                        offer</p>

                    <!-- SELECTED ITEMS GRID -->
                    <div class="grid grid-cols-4 gap-2" x-show="myOffer.length > 0">
                        <template x-for="item in myOffer" :key="item.id">
                            <div class="relative bg-[#1a1d2b] p-2 rounded cursor-pointer group"
                                @click="removeFromOffer(item)">
                                <img :src="item.image ? '/storage/' + item.image : 'https://via.placeholder.com/100'"
                                    class="w-full h-16 object-contain">
                                <div class="text-[10px] truncate text-gray-400 mt-1" x-text="item.name"></div>
                                <div class="text-[10px] text-green-400 font-bold" x-text="formatPrice(item.price)">
                                </div>
                                <div
                                    class="absolute inset-0 bg-red-500/20 opacity-0 group-hover:opacity-100 transition flex items-center justify-center rounded">
            ...
            <div class="flex flex-col gap-3 justify-center">
                <button
                    class="relative group overflow-hidden w-full py-4  font-bold text-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98]"
                    style="background: linear-gradient(135deg, #f97316 0%, #ea580c 100%); box-shadow: 0 10px 30px -10px rgba(249, 115, 22, 0.6);"
                    :disabled="!canTrade || isLoading" @click="submitTrade()">
                ...
                <div class="bg-[#151823] p-4  border border-white/5 text-center shadow-lg w-full">
                    <div
                        class="text-gray-400 text-[10px] uppercase tracking-wider mb-1 flex items-center justify-center gap-1 font-semibold">
            ...
            <div class="bg-[#12141c]  overflow-hidden flex flex-col">
                ...
                <div class="flex items-center justify-between p-4 bg-[#151823] border-b border-white/5 cursor-pointer select-none transition-colors hover:bg-[#1c202e]"
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <span class="bg-[#4c6fff] text-white text-xs font-bold px-2 py-0.5 rounded"
                            x-text="theirOffer.length" x-show="theirOffer.length > 0"></span>
                    </div>
                    <div class="flex items-center gap-4">
                        <button
                            class="text-xs text-[#aab0d5] hover:text-white px-2 py-1 rounded bg-[#1c202e] hover:bg-[#252a3d] transition-colors"
                ...
                <!-- Body -->
                <div x-show="receiveOpen" x-transition
                    class="p-4 bg-[#12141c] overflow-y-auto no-scrollbar transition-all duration-300"
                    :class="receiveExpanded ? 'min-h-[300px] max-h-[500px]' : 'min-h-[150px] max-h-[200px]'">
                    <p class="text-gray-400 text-sm text-center py-8" x-show="theirOffer.length === 0">Choose the
                        items you would like to receive</p>

                    <!-- SELECTED ITEMS GRID -->
                    <div class="grid grid-cols-4 gap-2" x-show="theirOffer.length > 0">
                        <template x-for="item in theirOffer" :key="item.id">
                            <div class="relative bg-[#1a1d2b] p-2 rounded cursor-pointer group"
                                @click="removeFromReceive(item)">
                                <img :src="item.image ? '/storage/' + item.image : 'https://via.placeholder.com/100'"
                                    class="w-full h-16 object-contain">
                                <div class="text-[10px] truncate text-gray-400 mt-1" x-text="item.name"></div>
                                <div class="text-[10px] text-green-400 font-bold" x-text="formatPrice(item.price)">
        ...
        <div class="grid grid-cols-[1fr_200px_1fr] gap-4 px-6 pb-10 items-stretch h-[calc(100vh-280px)] min-h-[500px]">
            ...
            <section class="bg-[#12141c]  flex flex-col h-full overflow-hidden min-h-0">
                ...
                <div class="h-[56px] flex items-center justify-between px-4 border-b border-white/5 gap-4">
                    <!-- Search Box -->
                    <div class="w-56 flex items-center gap-2 bg-[#0b0e14] px-3 py-2 rounded border border-white/5"
                        @click.stop>
                        <i class="fas fa-search text-gray-500 text-xs"></i>
                        <input
                            class="bg-transparent text-sm w-full outline-none text-gray-300 placeholder-gray-600 pointer-events-auto"
                ...
                <div class="flex-1 min-h-0 overflow-y-auto no-scrollbar bg-[#12141c] p-4" @scroll="closeDetail()">
                    ...
                    <div class="catalog-grid">
                            ...
                            <div class="catalog-col">
                                <div class="item-cell">
                                    <div class="item-details relative" @click="addToOffer(item)"
                                        :style="'box-shadow: 0 8px 24px ' + getRarityColor(item) + '33; border-color: ' +
                                            getRarityColor(item) + '22;'">
                                        <!-- Rarity badge overlay -->
                                        <div class="absolute left-2 top-2 z-30">
                                            <span class="text-xs font-semibold px-2 py-0.5 rounded max-w-full truncate"
                                                :style="'background: ' + getRarityColor(item) + '; color: ' +
                                                    getRarityTextColor(item) + '; box-shadow: 0 6px 16px ' +
                                                    getRarityColor(item) + '33;'">
                                        ...
                                        <div class="indicators">
                                            ...
                                            <div class="price font-rajdhani font-bold text-[#f97316]"
                                                x-text="formatPrice(item.price)"></div>
                                        </div>
                                        <div class="buttons z-20">
                                            <div class="btn-wrap">
                                                <button class="btn-unstack"
                                                    :style="isInOffer(item) ? 'background: #ef4444' :
                                                        'background: var(--primary)'"
                                                    @click.stop="addToOffer(item)">
                                                    <i class="fas"
                                                        :class="isInOffer(item) ? 'fa-times' : 'fa-plus'"></i>
                        ...
                        <div class="col-span-full text-center py-8 text-gray-400"
                            x-show="filteredMyInventory.length === 0">
                            ...
                            <template x-for="r in rarities" :key="r">
                                <div class="flex items-center gap-2 py-1 cursor-pointer hover:text-white text-gray-400 group/item"
                                    ...
                                    <div class="w-4 h-4 shrink-0 rounded-full border border-gray-600 flex items-center justify-center transition"
                                        ...
                                        <div class="w-1.5 h-1.5 bg-white rounded-full" x-show="selectedRarity === r">
                                        </div>
                                    </div>
                                ...
                                <div class="item-cell">
                                    <div class="item-details relative" @click="addToReceive(item)"
                                        ...
                                        <div class="buttons z-20">
                                            <div class="btn-wrap">
                                                <button class="btn-unstack"
                                                    :style="isInReceive(item) ? 'background: #ef4444' :
                                                        'background: var(--primary)'"
                                                    @click.stop="addToReceive(item)">
                                                    <i class="fas"
                                                        :class="isInReceive(item) ? 'fa-times' : 'fa-plus'"></i>
        ...
        <section class="w-full py-8 bg-[#12141c] mt-0 border-t border-white/5">
            <div class="max-w-[1920px] mx-auto px-6 space-y-6 text-gray-300 leading-relaxed">
                ...
                <p>Welcome to TEMAN, the premier platform for secure item trading. With thousands of successful trades
                    and a growing community of traders, we are dedicated to providing you with the fastest, most secure,
                    and reliable trading experience for your valuable items.</p>

                <h3 class="text-xl font-semibold text-white mt-8 mb-4">Why TEMAN Stands Out as the Best Trading
                    Platform:</h3>
                <p>Item trading is a serious business—every day, countless transactions happen worldwide. Trading items
                    is an essential part of the gaming and collecting experience. While there are many platforms
                    available, you need one you can truly trust.</p>

                <h4 class="text-lg font-semibold text-orange-500 mt-6 mb-2">Lightning-Fast Transactions:</h4>
                <p>Our system is built for speed. Execute trades quickly and efficiently, with access to a vast
                    inventory of premium items instantly.</p>

                <h4 class="text-lg font-semibold text-orange-500 mt-6 mb-2">Trusted by Thousands:</h4>
                <p>Our commitment to user satisfaction and reliability is unmatched, backed by positive reviews and a
                    growing user base.</p>

                <h4 class="text-lg font-semibold text-orange-500 mt-6 mb-2">Expert Item Knowledge:</h4>
                <p>Our detailed item guides and informative resources provide you with all the essential information to
                    make smart trading decisions.</p>

                <h4 class="text-lg font-semibold text-orange-500 mt-6 mb-2">24/7 Support:</h4>
                <p>Our dedicated support team is always available to help with any questions or issues, ensuring a
                    smooth and enjoyable trading experience.</p>

                <p>Our platform is user-friendly and has everything you need to trade items effectively. We're also
                    mobile-optimized, so you can trade from anywhere, anytime.</p>

                <h3 class="text-xl font-semibold text-white mt-8 mb-4">Fair Pricing, No Hidden Fees</h3>
                <p>We're becoming the go-to platform because we prioritize fair dealing. When you trade with us, you get
                    competitive pricing without the high commission fees that other platforms charge.</p>

                <p>We use market analysis to ensure fair pricing for all items, so both buyers and sellers get the best
                    possible deal. This means you get more value for your money and can build your collection
                    affordably.</p>
                ...
                <p>Whether you're a seasoned trader or just getting started, our platform is designed with simplicity in
                    mind.</p>

                <h4 class="text-lg font-semibold text-orange-500 mt-6 mb-2">Secure Transactions:</h4>
                <p>Your security is our top priority. Trade with confidence, knowing your transactions are fully
                    protected.</p>

                <h4 class="text-lg font-semibold text-orange-500 mt-6 mb-2">Stay Informed:</h4>
                <p>We regularly update our resources with guides, item evaluations, and the latest market trends. Stay
                    ahead of the curve with our expert insights.</p>

                <h3 class="text-xl font-semibold text-white mt-8 mb-4">Building a Trading Community:</h3>
                <p>Join our vibrant community of traders. Share experiences, get advice, and connect with fellow
                    enthusiasts.</p>
                ...
                <p>TEMAN is proud to be your trusted trading partner, offering an extensive selection of items available
                    for purchase, sale, and trade. Our intuitive platform combined with our commitment to excellent
                    service makes it easy to find what you're looking for. Our inventory updates regularly, so check
                    back often to discover new opportunities!</p>

                <h4 class="text-lg font-semibold text-orange-500 mt-6 mb-2">Trade with TEMAN:</h4>
                <p>At TEMAN, we're more than just a trading platform. We're a community of enthusiasts who understand
                    your needs and are committed to delivering exceptional trading services. Fast, reliable, and
                    knowledgeable—this is our promise to you.</p>

                <h4 class="text-lg font-semibold text-orange-500 mt-6 mb-2">Start Trading Now:</h4>
                <p>Dive into the exciting world of item trading with TEMAN. Experience our speed, reliability, and
                    expertise firsthand!</p>

                <h3 class="text-xl font-semibold text-white mt-8 mb-4">Frequently Asked Questions</h3>

                <h4 class="text-lg font-semibold text-orange-500 mt-6 mb-2">What are the best practices for safe item
                    trading?</h4>
                <p>For safe trading, always verify trading partners, use secure platforms like TEMAN, and follow
                    platform guidelines.</p>

                <h4 class="text-lg font-semibold text-orange-500 mt-6 mb-2">How does automated trading work?</h4>
                <p>Automated trading uses secure systems to facilitate fast, reliable transactions between users, acting
                    as a trusted intermediary.</p>

                <h4 class="text-lg font-semibold text-orange-500 mt-6 mb-2">How can I ensure a smooth trading
                    experience?</h4>
                <p>To ensure smooth trading, read item descriptions carefully, communicate clearly with trading
                    partners, and use our secure platform features.</p>
            </div>
        </section>
